...
#cgo LDFLAGS: -L"/Library/NDI SDK for Apple/lib/macOS" -lndi -Wl,-rpath,"/Library/NDI SDK for Apple/lib/macOS"
#include <stdbool.h>
#include <Processing.NDI.Lib.h>
#include <stdlib.h>

void set_stride(NDIlib_video_frame_v2_t *frame, int stride) {
...
// Init initializes the NDI SDK. Must be called once before using any NDI functions.
func Init() error {
...
// Destroy cleans up the NDI SDK.
func Destroy() {
	C.NDIlib_destroy()
}

// NewEncoder creates a new NDI sender with the specified name.
func NewEncoder(streamName string) (*Encoder, error) {
	...
	if sendInstance == nil {
		return nil, fmt.Errorf("failed to create NDI send instance")
	}
...
// SendVideoFrame sends a dynamically sized BGRA frame to NDI.
func (e *Encoder) SendVideoFrame(data []byte, width, height, stride int) {
	...
	videoFrame := C.NDIlib_video_frame_v2_t{
		xres:                 C.int(width),
		yres:                 C.int(height),
		FourCC:               C.NDIlib_FourCC_video_type_BGRA, // Match VideoToolbox output
		frame_rate_N:         30000,
		frame_rate_D:         1001,
	...
	C.set_stride(&videoFrame, C.int(stride))

	C.NDIlib_send_send_video_v2(e.sendInstance, &videoFrame)
}

// Close destroys the sender instance.
